...
else
{
   				...
   				{ 
						...
						$usuario2=$_SESSION['usuario'];
   						// Enviar consulta para insertar contacto en la agenda.
						 	...
						 	$instruccion="update playas set nombre_playa='".$_REQUEST['nombrel']."',longitud_playa='".$_REQUEST['apellidol']."' WHERE id_playa=$id";
						 	$consulta=mysql_query($instruccion,$conexion)
   					...
   					?>
   					<div class="titulomenu">¡Enorabuena ha actualizado satifactoriamente la playa!</div>
   					...
   					<div class="preguntalogin">Haz clik <a href="playas.php">aquí</a> para volver a las playas</div>
      				<?php
   				}
   				else
   				{
			?>
						...
						<div class="titulomenu">Actualizar Playa > </div><div class="imagentitulo"></div> 
						<div class="cuerpomenu">
						Cumplimenta este formulario para actualizar a una Playa
							<form ACTION="actualizarplaya.php?id=<?PHP print "$id"; ?>" METHOD="POST" ENCTYPE="multipart/form-data">
						<br/>
						<fieldset class="formulariol">
								Nombre de Playa *:<input type="text"  name="nombrel" value="<?PHP 
								if (isset($_REQUEST['enviarl']))
									print($_REQUEST['nombrel']);
								else
									print($resultado['nombre_playa']);
								?>"></input>
                    ...
                    if ($error['nombrel']!="")
							...
							<br/>
							<br/>
								Longitud *:<input type="number"  name="apellidol" value="<?PHP
                                ...
                                {
									print($resultado['longitud_playa']);
